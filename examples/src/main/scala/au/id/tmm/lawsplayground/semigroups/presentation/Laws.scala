...
object Laws {
  ...
  val associative: Law = new Law.With3Params(name = "associative") {
  ...
  val commutative: Law = new Law.With2Params(name = "commutative") {
    override def test[A: Instance](a1: A, a2: A): IsEq[A] =
  ...
  val associativeMultiplicativeBinaryOp: Law = new Law.With3Params(name = "associative multiplicative binary operation") {
    override def test[A: Instance](a1: A, a2: A, a3: A): IsEq[A] =
      ((a1 * a2) * a3) alwaysEquals (a1 * (a2 * a3))
  }
  ...
  val multiplicativeInverse: Law = new Law.With1Param(name = "multiplicative inverse") {
    override def test[A: Instance](a: A): IsEq[A] =
      a.`⁻¹` match {
        case Some(inverse) => (a * inverse) alwaysEquals one
        case None => pass
      }
  }
